...
function getBankNameByCardNo (cardNo) {
    var cardBin_6 = cardNo.substr(0, 6)
    var index = -1
    // 待优化：二分法
    for (var i = 0; i < cardBinArray.length; i++) {
        if (cardBinArray[i] == cardBin_6) {
            index = i
            break
        }
    }
    if (index != -1)
        return cardBinBankNameArray[index]
    var cardBin_8 = cardNo.substr(0, 8)
    for (var i = 0; i < cardBinArray.length; i++) {
    if (cardBinArray[i].length == 8 && cardBinArray[i] == cardBin_8) {
            index = i
            break
        }
    }
